...
std::tuple<py::array_t<int64_t> ,py::array_t<uint16_t> >receive_n(reuss::ZmqReceiver& r, int n_frames){
    ...
    py::array_t<uint16_t> data(std::array<size_t,2>{static_cast<size_t>(n_frames*G2_PACK),1280});
...
void init_zmqwriter(py::module &m) {
    ...
    ZmqReceiver.def(py::init<std::string, size_t>())
    .def("receive_n", &receive_n)
    .def("set_zmq_hwm", &reuss::ZmqReceiver::set_zmq_hwm);
}
